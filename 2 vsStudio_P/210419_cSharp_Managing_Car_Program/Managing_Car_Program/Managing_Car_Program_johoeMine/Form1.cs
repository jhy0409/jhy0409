    ...
    public partial class Form1 : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e) // 조회
        {
            ...
            try
            {
                ...
                //참조변수 람다개념 ------------------------------------------------------------------------
                ParkingCar car = DataManager.Cars.Single
                    ((x) => x.parkingSpot.ToString() == textBox5.Text); // Single : 일치하는거 하나 가져옴

                if (car.carNumber.Trim() == "") // 차 정보가 없음
                {
                    msg = "주차된 차가 없습니다.";
                    MessageBox.Show(msg);
                    writeLog(msg + ", " + textBox1.Text);
                ...
                {
                    string contens = $"주차공간 {car.parkingSpot}\n" +
                        $"차 번호\t{car.carNumber}\n" +
                        $"차주이름\t{car.driverName}\n" +
                        $"연락처\t{car.phoneNumber}\n" +
                        $"주차시간\t{car.parkingTime}";

                    string contens2 = $"주차공간 {car.parkingSpot}, 차 번호 {car.carNumber}, " +
                        $"차주이름 {car.driverName}, 연락처 {car.phoneNumber}, 주차시간 {car.parkingTime}";
                    MessageBox.Show(contens);
                    writeLog(contens2, DateTime.Now.ToString("yyyy_MM_dd"));
                }
            }
            catch (Exception ex)
            {
                string contents = $"{textBox5.Text}는 유효한 값이 아닙니다.";
                MessageBox.Show(contents);
                writeLog(contents);
                writeLog(ex.Message);
                writeLog(ex.StackTrace);
